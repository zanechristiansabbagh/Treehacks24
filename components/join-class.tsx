...
export const JoinClass = ({ parsedEmail }) => {
  ...
  const createNewClassIfNotExists = useMutation(
    api.classes.createNewClassIfNotExists
  );
  ...
  const handlePhoneChange = async (e) => {
    const formattedValue = e.target.value
      .replace(/[^\d]/g, "")
      .replace(/(\d{1,3})(\d{0,3})(\d{0,4})/, (match, p1, p2, p3) =>
  ...
  const handleSubmit = async (e) => {
    ...
    console.log(
      "Submission Status:",
      isSubmitted ? "Not Submitted" : "Submitted"
    ); // Log the submission status, toggled from its previous state

    console.log(getClassesByEmail, "classID");
    const studentId = await createNewStudent({
      studentName: nameValue,
      studentPhoneNumber: cleanNumber,
      classId: getClassesByEmail,
    });
    ...
    return (
      ...
      >
        ...
        <h1 className="text-7xl font-extrabold mb-10">
          <span className="text-orange-500">Ta.ai</span> class
        </h1>
          ...
          <form onSubmit={handleSubmit} className="w-full items-center gap-2">
            ...
            <div className="mt-8">
              <button
                ...
                } text-white rounded-full text-xl shadow-lg self-center`}
              >
                Submit
              </button>
            </div>
